...
def before_all(context):
    ...
    context.driver = webdriver.Remote('http://172.10.20.61:4271/wd/hub',desired_caps)#初始化手机
    image_path = ".\\test_group\\port_4271\\image"
    report_path = ".\\test_group\\port_4271\\report"
    video_path = ".\\test_group\\port_4271\\video"
    ...
    os.mkdir(video_path)
    with open(".\\templates\\4271.html", "w") as f:  #制作报告
...
def after_all(context):
    ...
    #pass
    with open(".\\templates\\4271.html","a+") as f:
        now_time = datetime.datetime.now()
        script = u"""<script>$('#report_status').append('<p class="attribute"><strong>结束时间 : </strong> {end_time}</p>')</script>""".format(
    ...
    # os.system(close_shell)
    sendQYWechatMSG("4271",appPackage)
...
def after_scenario(context, scenario):
    ...
    video_id = picvideo(port="4271",path=".\\test_group\\port_4271\\image",video_path=".\\test_group\\port_4271\\video")
    with open(".\\templates\\4271.html","a+") as f:
        f.write(end_Scenario_template.format(Activity=end_activity,video_id=video_id).encode("utf8"))
def after_step(context,step):
    """每步完成后进行图片保存,日志表记录"""
    port = "4271"
    file_name = "4271_%d"%(time.time())
    file_path_name = ".\\test_group\\port_4271\\image\\"+file_name
    with open(".\\templates\\4271.html","a+") as f:
    ...
    img = context.driver.get_screenshot_as_file('.\\test_group\\port_4271\\image\\%s.png'%(file_name))
